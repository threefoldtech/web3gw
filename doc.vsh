...
fn build_openrpc_docs() ! {
	...
	sh('${cli} docgen -t "Web3Proxy JSON-RPC API" -p -o docs/openrpc web3gw/client')
...
fn build_playground(url string) ! {
	...
	modified_package_json := package_json.replace('"homepage": "https://playground.open-rpc.org/"',
		'"homepage": "${url}/playground"')
